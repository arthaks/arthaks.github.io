...
- 只有用户明确调用 `/skill:life-activity-journal`，或者明确说“更新生活轨迹”时，才允许修改生活轨迹数据和年度页面。
- 用户仅发送截图、运动数据、读书内容、随笔素材或书籍内容，不代表要更新生活轨迹；不明确时先询问。
- 普通博客创作、技术文章和个人书籍发布与生活轨迹相互独立，不得自动合并。
- 生活轨迹 Skill 使用命令模式，禁止模型自动调用。
